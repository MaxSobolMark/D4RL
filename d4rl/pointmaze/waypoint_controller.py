...
class WaypointController(object):
    def __init__(
        ...
        self.target_to_q_values = target_to_q_values
    ...
    def _new_target(self, start, target):
        ...
        if target in self.target_to_q_values:
            q_values = self.target_to_q_values[target]
        else:
            q_values = q_iteration.q_iteration(env=self.env, num_itrs=50, discount=0.99)
            self.target_to_q_values[target] = q_values
